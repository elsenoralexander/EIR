    ...
    return (
        <div className="min-h-screen bg-[#020617] text-white p-4 sm:p-10 font-sans">
            <div className="max-w-7xl mx-auto space-y-10">
                ...
                {/* Tab Switcher */}
                <div className="flex items-center gap-2 p-1 bg-white/5 rounded-3xl border border-white/10 w-fit">
                    <button
                        onClick={() => setActiveTab('pedidos')}
                        className={`px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all flex items-center gap-2 ${activeTab === 'pedidos'
                    ...
                    <button
                        ...
                        className={`px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all flex items-center gap-2 ${activeTab === 'sugerencias'
                    ...
                    {activeTab === 'pedidos' ? (
                        ...
                        <>
                            <div className="glass-panel p-8 rounded-[40px] border border-white/5 flex items-center gap-6 shadow-2xl relative overflow-hidden group">
                                ...
                                <div>
                                    <p className="text-[10px] font-black text-amber-500/40 uppercase tracking-widest mb-1">Mensajes Totales</p>
                                    <p className="text-4xl font-display font-black">{suggestions.length}</p>
                                </div>
                            </div>
                            <div className="glass-panel p-8 rounded-[40px] border border-white/5 flex items-center gap-6 shadow-2xl relative overflow-hidden group col-span-2">
                                <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/5 to-transparent" />
                                <div className="p-5 rounded-3xl bg-emerald-500/10 text-emerald-400 group-hover:scale-110 transition-transform">
                                    <Sparkles className="w-8 h-8" />
                                </div>
                                <div>
                                    <p className="text-[10px] font-black text-emerald-500/40 uppercase tracking-widest mb-1">Estado del Oráculo</p>
                                    <p className="text-xl font-display font-black text-emerald-400">EVOLUCIONANDO CON EL FEEDBACK</p>
                                </div>
                            </div>
                        </>
                    )}
                ...
                <div className="glass-panel rounded-[40px] border border-white/5 overflow-hidden shadow-2xl bg-white/5">
                    <div className="p-8 border-b border-white/5 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            {activeTab === 'pedidos' ? <FileText className="w-5 h-5 text-emerald-400" /> : <MessageSquare className="w-5 h-5 text-amber-400" />}
                            <h3 className="text-lg font-display font-black text-white uppercase tracking-widest">
                                {activeTab === 'pedidos' ? 'HISTORIAL DETALLADO' : 'BUZÓN DE SUGERENCIAS'}
                            </h3>
                        </div>
                        {activeTab === 'pedidos' ? (
                            ...
                            <span className="text-[10px] font-bold text-amber-500/40 bg-amber-500/5 px-4 py-1.5 rounded-full border border-amber-500/10 uppercase tracking-widest">
                                {suggestions.length} Mensajes
                            </span>
                        )}
                    </div>
                    ...
                    <div className="overflow-x-auto">
                        {activeTab === 'pedidos' ? (
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-white/5">
                                        <th className="px-8 py-5 text-[10px] font-black text-amber-500/60 uppercase tracking-widest w-48">Fecha y Hora</th>
                                        <th className="px-8 py-5 text-[10px] font-black text-amber-500/60 uppercase tracking-widest">Sugerencia / Feedback</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-white/5">
                                    {loading ? (
                                        <tr>
                                            <td colSpan={2} className="px-8 py-20 text-center">
                                                <div className="flex flex-col items-center gap-4">
                                                    ...
                                                    <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Leyendo Pensamientos...</p>
                                                </div>
                                            </td>
                                        </tr>
                                    ) : suggestions.length > 0 ? suggestions.map((suggestion) => (
                                        <tr key={suggestion.id} className="hover:bg-white/5 transition-colors group">
                                            <td className="px-8 py-8 align-top">
                                                <div className="flex items-center gap-3 mb-1">
                                                    <Clock className="w-3.5 h-3.5 text-amber-500/40" />
                                                    <p className="text-xs font-bold text-white">
                                                        {new Date(suggestion.createdAt).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit' })}
                                                    </p>
                                                </div>
                                                <p className="text-[10px] text-slate-500 font-medium pl-6">
                                                    {new Date(suggestion.createdAt).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}
                                                </p>
                                            </td>
                                        ...
                                        <tr>
                                            <td colSpan={2} className="px-8 py-20 text-center">
                                                <p className="text-slate-500 text-xs font-medium italic">Todavía no hay sugerencias en el buzón.</p>
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
